...
export default tool({
  ...
  args: {
    url: tool.schema
      .string()
      .describe("The request URL (e.g., 'https://api.example.com/users')"),
    method: tool.schema
      .string()
      .optional()
      .describe(
        "HTTP method (GET, POST, PUT, DELETE, etc.). Defaults to GET (or POST if data is provided).",
      ),
    headers: tool.schema
      .array(tool.schema.string())
      .optional()
      .describe(
        "HTTP headers in 'Name:Value' format (e.g., ['Authorization:Bearer token', 'Accept:application/json'])",
      ),
    data: tool.schema
      .array(tool.schema.string())
      .optional()
    ...
    params: tool.schema
      ...
      .optional()
      .describe(
        "URL parameters in 'key==value' format to be appended to the URL.",
      ),
    flags: tool.schema
      .array(tool.schema.string())
      .optional()
  ...
  async execute({ url, method, headers, data, params, flags, print }) {
    ...
    if (headers) {
      for (const h of headers) {
        ...
        if (h.includes("@"))
          return `Error: '@' operator is prohibited in headers. Item: "${h}"`;
        httpArgs.push(h);
      }
    }
    ...
    if (data) {
      for (const d of data) {
        ...
        if (d.includes("@"))
          return `Error: '@' operator is prohibited in data fields. Item: "${d}"`;
        httpArgs.push(d);
      }
    }
    ...
    if (params) {
      for (const p of params) {
        ...
        if (p.includes("@"))
          return `Error: '@' operator is prohibited in parameters. Item: "${p}"`;
        httpArgs.push(p);
      }
    }
    ...
    try {
      ...
      if (
        stderr.includes("ConnectionError") ||
      ...
      const lines = stderr
        ...
        .filter(Boolean);
      const lastLine =
        lines.find((l) => l.toLowerCase().startsWith("http: error:")) ||
